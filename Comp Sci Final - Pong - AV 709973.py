...
#that has 3 levels (beginner, regular, challenging) to choose. Using the left
...
img1 = PhotoImage(file='C:/Users/foram/Downloads/Comp Sci Final - Pong Game - AV 709973/pingpong0.png', width = 300, height=300)
...
def open():
    class ball: #blueprint for the object "ball"
        def __init__ (self, canvas, size, paddle):
            self.canvas = canvas
            self.paddle = paddle
            self.id = canvas.create_rectangle(10,10,size,size, fill="white")
    ...
    canvas = Canvas(game, width=500, height=500, bd=0, bg='light blue')
    canvas.pack()
    label = canvas.create_text(5, 5, anchor=NW, text="Score: 0")
    paddle = Paddle(canvas)
    ball = ball(canvas,30,paddle)
    ...
    while ball.touch_bottom == False:
        ...
        time.sleep(0.01)#suspends the game's exectution for 0.01 seconds
...
def open():
    ...
    canvas = Canvas(game_med, width=500, height=500, bd=0, bg='light blue')
    canvas.pack()
    label = canvas.create_text(5, 5, anchor=NW, text="Score: 0")
    paddle = Paddle(canvas)
    ball = ball(canvas,30,paddle)
...
def open():
    ...
    game_hard.configure(bg="black")
    canvas = Canvas(game_hard, width=500, height=500, bd=0, bg='light blue')
    canvas.pack()
    label = canvas.create_text(5, 5, anchor=NW, text="Score: 0")
    paddle = Paddle(canvas)
    ball = ball(canvas,30,paddle)
